...
public final class GGUFLoader {
    ...
    public static AccelTensor loadTensorF32(GGUFModel model, String name) {
        GGUFTensorInfo info = model.tensors().stream()
            .filter(t -> t.name().equals(name))
            .findFirst()
            .orElseThrow(() -> new IllegalArgumentException("Tensor not found: " + name));
        return loadTensorF32(model, info);
    }

    public static AccelTensor loadTensorF32Optional(GGUFModel model, String name) {
        GGUFTensorInfo info = model.tensors().stream()
            .filter(t -> t.name().equals(name))
            .findFirst()
            .orElse(null);
        return info == null ? null : loadTensorF32(model, info);
    }

    private static AccelTensor loadTensorF32(GGUFModel model, GGUFTensorInfo info) {
        MemorySegment raw = model.segment().asSlice(model.dataStart() + info.offset(), info.sizeInBytes());
        long[] shape = info.shape();
        long numElements = 1;
        for (long d : shape) numElements *= d;

        if (info.typeId() == 0) { // F32
            return AccelTensor.wrapSegment(raw, shape);
        }

        AccelTensor f32 = AccelTensor.zeros(shape);
        MemorySegment out = f32.dataSegment();

        if (info.typeId() == 1) { // F16
            Dequantizer.dequantizeF16(raw, out, numElements);
        } else if (info.typeId() == 8) { // Q8_0
            Dequantizer.dequantizeQ8_0(raw, out, numElements);
        } else if (info.typeId() == 2) { // Q4_0
            Dequantizer.dequantizeQ4_0(raw, 0, out, numElements);
        } else if (info.typeId() == 12) { // Q4_K
            Dequantizer.dequantizeQ4_K(raw, out, numElements);
        } else if (info.typeId() == 13) { // Q5_K
            Dequantizer.dequantizeQ5_K(raw, out, numElements);
        } else if (info.typeId() == 14) { // Q6_K
            Dequantizer.dequantizeQ6_K(raw, out, numElements);
        } else {
            f32.close();
            throw new IllegalArgumentException("Unsupported GGUF tensor type id " + info.typeId() + " for " + info.name());
        }
        return f32;
    }
