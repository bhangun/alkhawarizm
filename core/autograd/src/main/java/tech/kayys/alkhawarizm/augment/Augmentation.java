package tech.kayys.alkhawarizm.augment;
...
public interface Augmentation {
    ...
    class RandomHorizontalFlip implements Augmentation {
        ...
        private RandomHorizontalFlip(double probability) {
            this.probability = probability;
        }

        public static RandomHorizontalFlip of(double probability) {
            return new RandomHorizontalFlip(probability);
        }

        public static RandomHorizontalFlip create() {
            return new RandomHorizontalFlip(0.5);
        }
    ...
    class RandomVerticalFlip implements Augmentation {
        ...
        private RandomVerticalFlip(double probability) {
            this.probability = probability;
        }

        public static RandomVerticalFlip of(double probability) {
            return new RandomVerticalFlip(probability);
        }
    ...
    class RandomCrop implements Augmentation {
        ...
        private RandomCrop(int size, int padding) {
            this.size = size;
            this.padding = padding;
        }

        public static RandomCrop of(int size) {
            return new RandomCrop(size, size / 8);
        }

        public static RandomCrop of(int size, int padding) {
            return new RandomCrop(size, padding);
        }
        ...
        public GradTensor apply(GradTensor input, Random rng) {
            ...
            for (int c = 0; c < C; c++)
                for (int h = 0; h < cropH; h++)
                    System.arraycopy(padded, c * pH * pW + (oh + h) * pW + ow, out, c * cropH * cropW + h * cropW,
                            cropW);
    ...
    class ColorJitter implements Augmentation {
        ...
        public static ColorJitter of(double b, double c, double s) {
            return new ColorJitter(b, c, s);
        }
    ...
    class RandomErasing implements Augmentation {
        ...
        public static RandomErasing of(double probability) {
            return new RandomErasing(probability, 0.02, 0.3);
        }
    ...
    class RandomGrayscale implements Augmentation {
        ...
        private RandomGrayscale(double probability) {
            this.probability = probability;
        }

        public static RandomGrayscale of(double probability) {
            return new RandomGrayscale(probability);
        }
        ...
        public GradTensor apply(GradTensor input, Random rng) {
            ...
            for (int h = 0; h < H; h++)
                for (int w = 0; w < W; w++) {
                    float gray = 0.299f * src[h * W + w] + 0.587f * src[H * W + h * W + w]
                            + 0.114f * src[2 * H * W + h * W + w];
                    for (int c = 0; c < C; c++)
                        dst[c * H * W + h * W + w] = gray;
                }
