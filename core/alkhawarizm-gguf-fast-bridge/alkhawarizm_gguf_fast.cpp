...
static void alkhawarizm_llama_log_callback(ggml_log_level level, const char *text, void *) {
...
static int tokenize_prompt(
    ...
    const char *prompt,
    int prompt_len) {
  const bool token_cache_allowed = env_flag_enabled("ALKHAWARIZM_GGUF_FAST_TOKEN_CACHE", true);
...
static bool save_prompt_prefix_cache(
  ...
  handle->prompt_cache_flags = env_flag_enabled("ALKHAWARIZM_GGUF_FAST_PROMPT_CACHE_ON_DEVICE", true)
...
static bool save_shared_prompt_prefix_cache(
  ...
  handle->shared_prompt_cache_flags = env_flag_enabled("ALKHAWARIZM_GGUF_FAST_SHARED_PROMPT_CACHE_ON_DEVICE", false)
      ? LLAMA_STATE_SEQ_FLAGS_ON_DEVICE
      : LLAMA_STATE_SEQ_FLAGS_NONE;
...
void *alkhawarizm_gguf_open(const char *model_path,
  ...
  std::call_once(backend_once, [backend_dir]() {
    if (!env_flag_enabled("ALKHAWARIZM_GGUF_FAST_RUN_DEBUG")) {
      llama_log_set(alkhawarizm_llama_log_callback, nullptr);
    }
  ...
  auto *handle = new alkhawarizm_gguf_ctx();
  ...
  if (n_gpu_layers != 0 && !env_flag_enabled("ALKHAWARIZM_GGUF_NO_FLASH_ATTN")) {
...
int alkhawarizm_gguf_generate(void *raw_handle,
                         const char *prompt,
                         int max_tokens,
                         float temperature,
                         int top_k,
  ...
  auto *handle = static_cast<alkhawarizm_gguf_ctx *>(raw_handle);
  ...
  const int prompt_cache_min_tokens = env_int("ALKHAWARIZM_GGUF_FAST_PROMPT_CACHE_MIN_TOKENS", 128);
  handle->last_prompt_cache_min_tokens = prompt_cache_min_tokens;
  const int shared_prompt_cache_min_tokens = env_int("ALKHAWARIZM_GGUF_FAST_PROMPT_CACHE_SHARED_MIN_TOKENS", 64);
  const bool prompt_cache_allowed = env_flag_enabled("ALKHAWARIZM_GGUF_FAST_PROMPT_CACHE", true);
  const int repeat_prompt_cache_min_tokens = env_int("ALKHAWARIZM_GGUF_FAST_PROMPT_CACHE_REPEAT_MIN_TOKENS", 8);
  ...
  const bool eager_short_prompt_cache_enabled =
      env_flag_enabled("ALKHAWARIZM_GGUF_FAST_PROMPT_CACHE_EAGER_SHORT", true)
  ...
  if (prompt_cache_enabled) {
    ...
    if (!prompt_ready) {
      ...
      const int shared_prefix_tokens = std::min(
          n_prompt - 1,
          env_int("ALKHAWARIZM_GGUF_FAST_PROMPT_CACHE_SHARED_TOKENS", 64));
...
int alkhawarizm_gguf_last_metrics(void *raw_handle, char *output, size_t output_size) {
...
const char *alkhawarizm_gguf_backend_name(void *raw_handle) {
  if (raw_handle == nullptr) {
    return "unknown";
  }
  auto *handle = static_cast<alkhawarizm_gguf_ctx *>(raw_handle);
...
void alkhawarizm_gguf_hard_exit(int status) {
  std::fflush(stdout);
  std::fflush(stderr);
  std::_Exit(status);
}

void alkhawarizm_gguf_close(void *raw_handle) {
  auto *handle = static_cast<alkhawarizm_gguf_ctx *>(raw_handle);
  delete handle;
}
