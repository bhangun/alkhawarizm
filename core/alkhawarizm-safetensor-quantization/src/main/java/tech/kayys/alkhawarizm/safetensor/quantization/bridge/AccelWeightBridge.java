 ...
 */
package tech.kayys.alkhawarizm.safetensor.quantization.bridge;
...
public class AccelWeightBridge {
    ...
    private AccelTensor attachQuantizationMetadata(AccelTensor t, String weightName, SafetensorShardSession session) {
        ...
        if (session.findTensor(scalesName).isPresent()) {
            ...
            if (st.dtype() == tech.kayys.alkhawarizm.safetensor.loader.SafetensorDType.I32 ||
                st.dtype() == tech.kayys.alkhawarizm.safetensor.loader.SafetensorDType.U32 ||
                (st.dtype() == tech.kayys.alkhawarizm.safetensor.loader.SafetensorDType.I8 && weightName.endsWith(".qweight"))) {
                type = AccelTensor.QuantType.INT4;
            } else if (st.dtype() == tech.kayys.alkhawarizm.safetensor.loader.SafetensorDType.U8 && weightName.endsWith(".qweight")) {
            ...
            if (type == AccelTensor.QuantType.NF4 || type == AccelTensor.QuantType.INT4) {
                ...
                if (st.dtype() == tech.kayys.alkhawarizm.safetensor.loader.SafetensorDType.U8 ||
                    st.dtype() == tech.kayys.alkhawarizm.safetensor.loader.SafetensorDType.I8) {
                    logicalShape[logicalShape.length - 1] *= 2;
                } else if (st.dtype() == tech.kayys.alkhawarizm.safetensor.loader.SafetensorDType.I32 ||
                           st.dtype() == tech.kayys.alkhawarizm.safetensor.loader.SafetensorDType.U32) {
