package tech.kayys.alkhawarizm.cuda;
 ...
 * array
 * fragmentation via Foreign Function offset iterations over the unified CUDA
 * Binding.
 */
public class CudaComputeBackend implements ComputeBackend {
    ...
    private ScratchBuffers ensureBuffers(long reqA, long reqB, long reqC) {
        ...
        if (buffers == null || buffers.memA.byteSize() < reqA || buffers.memB.byteSize() < reqB
                || buffers.memC.byteSize() < reqC) {
            if (buffers != null)
                buffers.close();
