package tech.kayys.alkhawarizm.cuda.runner;
...
import tech.kayys.alkhawarizm.cuda.detection.CudaCapabilities;
 ...
 * <p>
 * This runner loads a pre-compiled CUDA kernel module ({@code .cubin} file)
 * compiled from custom CUDA kernels. The forward pass follows the same
 * structure as
 * {@link tech.kayys.alkhawarizm.extension.metal.runner.MetalRunner}:
 * <ol>
 * <li>RMS Norm — {@code alkhawarizm_cuda_rmsnorm}</li>
 * <li>QKV projection — {@code alkhawarizm_cuda_matmul} (cuBLAS/cuBLASLt)</li>
 * <li>FlashAttention-2/3 — {@code alkhawarizm_cuda_flash_attn_v2/v3}</li>
 * <li>SiLU FFN — {@code alkhawarizm_cuda_silu_ffn}</li>
 * </ol>
 *
 * <h2>FlashAttention Support</h2>
 * <p>
 * On A100+ (compute capability ≥ 8.0), FlashAttention-2 is used for
 * fused QK^T/softmax/×V in a single GPU pass. On H100+ (≥ 9.0),
 * FlashAttention-3 adds FP8 tensor core acceleration.
 *
 * <h2>Memory Model</h2>
 * <p>
 * On A100/H100 with unified memory architecture,
 * {@link CudaBinding#mallocManaged}
 ...
 * 
 * <pre>
 *   alkhawarizm.runners.cuda.enabled=false
 *   alkhawarizm.runners.cuda.mode=auto  # auto|standard|offload|force|disabled
 ...
 *   # Output: target/native/linux-x86_64/alkhawarizm_cuda_sm80.cubin
 * </pre>
 */
@ApplicationScoped
public class CudaRunner extends AbstractAljabrRunner {
    ...
    @ConfigProperty(name = "alkhawarizm.runners.cuda.enabled", defaultValue = "false")
    boolean enabled;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.mode", defaultValue = "auto")
    String cudaMode;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.library-path", defaultValue = "/usr/local/cuda/lib64/libalkhawarizm_cuda.so")
    String libraryPath;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.device-id", defaultValue = "0")
    int deviceId;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.num-layers", defaultValue = "32")
    int numLayers;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.num-heads", defaultValue = "32")
    int numHeads;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.num-heads-kv", defaultValue = "8")
    int numHeadsKv;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.head-dim", defaultValue = "128")
    int headDim;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.model-dim", defaultValue = "4096")
    int modelDim;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.ffn-dim", defaultValue = "14336")
    int ffnDim;

    @ConfigProperty(name = "alkhawarizm.runners.cuda.vocab-size", defaultValue = "32000")
    int vocabSize;
    ...
    public void initialize(ModelManifest modelManifest, RunnerConfiguration config)
        ...
        if (!enabled)
            throw new RunnerInitializationException(
                    ...
                    "CUDA runner disabled (alkhawarizm.runners.cuda.enabled=false)");
        if (mode == CudaRunnerMode.DISABLED)
            throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "CUDA runner disabled (alkhawarizm.runners.cuda.mode=disabled)");
        if (mode == CudaRunnerMode.OFFLOAD)
            throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "CUDA runner disabled (alkhawarizm.runners.cuda.mode=offload)");
     ...
     * <li>FlashAttention — {@link CudaBinding#flashAttnV2} or
     * {@link CudaBinding#flashAttnV3}</li>
            ...
            try {
                ...
                for (int step = 0; step < maxTok; step++) {
                    ...
                    emitter.emit(new StreamingInferenceChunk(
                            reqId, seq++, ModalityType.TEXT, detokenize(next), null, fin, fin ? "stop" : null, null,
