package tech.kayys.alkhawarizm.rocm.runner;
 ...
 * <p>
 * This runner loads a pre-compiled HIP kernel module ({@code .hsaco} file)
 ...
 * {@link tech.kayys.alkhawarizm.extension.metal.runner.MetalRunner} but uses
 * HIP
 * instead of Metal:
 * <ol>
 * <li>RMS Norm — {@code alkhawarizm_rocm_rmsnorm}</li>
 * <li>QKV projection — {@code alkhawarizm_rocm_gemm}</li>
 * <li>Paged attention — {@code alkhawarizm_rocm_paged_attention} (hipified
 * FA)</li>
 * <li>SiLU FFN — {@code alkhawarizm_rocm_silu_ffn}</li>
 * </ol>
 *
 * <h2>MI300X memory model</h2>
 * <p>
 * The AMD MI300X (and MI300A) uses a unified CPU+GPU memory architecture
 ...
 * correct file is selected at runtime by reading the device's
 * {@code gcnArchName}.
 *
 * <h3>Config</h3>
 * 
 * <pre>
 *   alkhawarizm.runners.rocm.enabled=false
 *   alkhawarizm.runners.rocm.library-path=/opt/rocm/lib/libamdhip64.so
 *   alkhawarizm.runners.rocm.kernel-path=/opt/alkhawarizm/lib/alkhawarizm_rocm_gfx942.hsaco
 *   alkhawarizm.runners.rocm.device-id=0
 *   alkhawarizm.runners.rocm.num-layers=32
 *   alkhawarizm.runners.rocm.num-heads=32
 *   alkhawarizm.runners.rocm.num-heads-kv=8
 *   alkhawarizm.runners.rocm.head-dim=128
 *   alkhawarizm.runners.rocm.model-dim=4096
 *   alkhawarizm.runners.rocm.ffn-dim=14336
 *   alkhawarizm.runners.rocm.vocab-size=32000
 * </pre>
 *
 ...
 *   # Output: target/native/linux-x86_64/alkhawarizm_rocm_gfx942.hsaco
 * </pre>
 */
@ApplicationScoped
public class RocmRunner extends AbstractAljabrRunner {
    ...
    @ConfigProperty(name = "alkhawarizm.runners.rocm.enabled", defaultValue = "false")
    boolean enabled;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.library-path", defaultValue = "/opt/rocm/lib/libamdhip64.so")
    String libraryPath;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.kernel-path", defaultValue = "/opt/alkhawarizm/lib/alkhawarizm_rocm_gfx942.hsaco")
    String kernelPath;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.device-id", defaultValue = "0")
    int deviceId;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.num-layers", defaultValue = "32")
    int numLayers;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.num-heads", defaultValue = "32")
    int numHeads;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.num-heads-kv", defaultValue = "8")
    int numHeadsKv;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.head-dim", defaultValue = "128")
    int headDim;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.model-dim", defaultValue = "4096")
    int modelDim;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.ffn-dim", defaultValue = "14336")
    int ffnDim;

    @ConfigProperty(name = "alkhawarizm.runners.rocm.vocab-size", defaultValue = "32000")
    int vocabSize;
    ...
    public void initialize(ModelManifest modelManifest, RunnerConfiguration config)
        ...
        if (!enabled)
            throw new RunnerInitializationException(
                    ...
                    "ROCm runner disabled (alkhawarizm.runners.rocm.enabled=false)");
        ...
        if (!isNull(hipModule)) {
            fnRmsNorm = hip.moduleGetFunction(hipModule, "alkhawarizm_rocm_rmsnorm");
            fnGemm = hip.moduleGetFunction(hipModule, "alkhawarizm_rocm_gemm");
            fnPagedAttention = hip.moduleGetFunction(hipModule, "alkhawarizm_rocm_paged_attention");
            fnSiluFfn = hip.moduleGetFunction(hipModule, "alkhawarizm_rocm_silu_ffn");
        ...
        if (isUnifiedMemory) {
            ...
            dQkv = hip.mallocManaged((long) (numHeads + 2 * numHeadsKv) * headDim * elemBytes,
                    RocmHipBinding.HIP_MEM_ATTACH_GLOBAL);
     ...
     *
     * <p>
     * On MI300X (unified memory) all tensor reads/writes happen in-place
     ...
     *
     * <p>
     * Each layer executes four HIP kernels via
    ...
    private float[] runForwardPass(int seqLen, int[] bt, boolean decodeOnly) {
        ...
        for (int layer = 0; layer < numLayers; layer++) {
            ...
            if (!isUnifiedMemory && devWeights != layerWeights)
                hip.free(devWeights);
        }
     ...
     * HIP requires an array of pointers-to-values as the {@code kernelParams}
     * argument.
     */
    private MemorySegment packKernelArgs(Arena arena, Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            Object arg = args[i];
            if (arg instanceof MemorySegment ms)
                slot.set(ValueLayout.ADDRESS, 0L, ms);
            else if (arg instanceof Integer iv)
                slot.setAtIndex(ValueLayout.JAVA_INT, 0, iv);
            else if (arg instanceof Long lv)
                slot.setAtIndex(ValueLayout.JAVA_LONG, 0, lv);
            else if (arg instanceof Float fv)
                slot.setAtIndex(ValueLayout.JAVA_FLOAT, 0, fv);
