/**
 * aljabr_metal_fa4.m — Metal FA4 bridge for Aljabr
 *
 ...
 * globals g_device and g_queue from aljabr_metal_support.m, while CPU fallback
 * kernels live in aljabr_metal_cpu_fallback.m.
 */
 ...
 */
int aljabr_metal_fa4_attention(
        void*        output,
        const void*  query,
        const void*  key,
        const void*  value,
    ...
    for (int b = 0; b < B; b++) {
        for (int h = 0; h < H; h++) {
            ...
            if (mps_matmul_tb_f32(scorePtr, qPtr, kPtr, T, D, S, scale) != 0) {
                aljabr_metal_cpu_matmul_tb(scorePtr, qPtr, kPtr, T, D, S, scale, 0.0f);
            }
            ...
            if (mps_matmul_nn_f32(oPtr, scorePtr, vPtr, T, S, D, 1.0f) != 0) {
                aljabr_metal_cpu_matmul_nn(oPtr, scorePtr, vPtr, T, S, D, 1.0f, 0.0f);
            }
            free(scorePtr);
        }
    }
 ...
 */
int aljabr_metal_fa4_sdpa_available(void) {
 ...
 */
int aljabr_metal_fa4_bf16_available(void) {
