/**
 * aljabr_metal_attention.m — paged attention bridge implementation.
 */
...
static int aljabr_metal_decode_attention_gpu(
        void* out,
    ...
    id<MTLComputePipelineState> pipeline = aljabr_metal_pipelines()->decode_attention;
    if (!g_initialized || pipeline == nil) return -3;
    if (aljabr_metal_env_truthy("ALJABR_METAL_DISABLE_DECODE_ATTENTION_KERNEL")) return -9;
    if (B <= 0 || H <= 0 || H_kv <= 0 || D <= 0 || block_size <= 0 || max_blocks <= 0) return -2;
    if (H % H_kv != 0) return -2;
    if (D > 256) return -8;

    int max_context = aljabr_metal_env_int_or_default("ALJABR_METAL_DECODE_ATTENTION_MAX_CONTEXT", 4096);
...
static int aljabr_metal_attention_impl(
        void* out,
        const void* Q, const void* K_cache, const void* V_cache,
        const int* block_table, const int* context_lens,
    ...
    if (T == 1) {
        int gpu_rc = aljabr_metal_decode_attention_gpu(
        ...
        if (gpu_rc != -9 && aljabr_metal_env_truthy("ALJABR_METAL_DECODE_ATTENTION_DEBUG")) {
            fprintf(stderr,
                    "[aljabr-metal] decode attention GPU kernel unavailable rc=%d; falling back to CPU attention bridge\n",
                    gpu_rc);
        }
    }
    BOOL use_decode_sgemv = (T == 1) && !aljabr_metal_env_truthy("ALJABR_METAL_DISABLE_ATTENTION_SGEMV");
    ...
    for (int b = 0; b < B; b++) {
        ...
        if (!aljabr_metal_ensure_attention_scratch(scratch_elements, &kPtr, &vPtr, &scoreBuf)) {
            return -4;
        }
...
int aljabr_metal_attention_windowed(
        void* out,
        const void* Q, const void* K_cache, const void* V_cache,
        const int* block_table, const int* context_lens,
        int B, int T, int H, int D, int block_size, int max_blocks,
...
int aljabr_metal_attention_gqa(
        void* out,
        const void* Q, const void* K_cache, const void* V_cache,
        const int* block_table, const int* context_lens,
        int B, int T, int H, int H_kv, int D, int block_size, int max_blocks,
...
int aljabr_metal_attention_gqa_windowed(
        void* out,
        const void* Q, const void* K_cache, const void* V_cache,
        const int* block_table, const int* context_lens,
        int B, int T, int H, int H_kv, int D, int block_size, int max_blocks,
