/**
 * aljabr_metal_matvec_api.m — exported Metal matvec C API for Aljabr.
 * 
...
#import "aljabr_metal_attention.h"
#import "aljabr_metal_buffers.h"
#import "aljabr_metal_scratch.h"
#import "aljabr_metal_mps_cache.h"
#import "aljabr_metal_mps_validation.h"
#import "aljabr_metal_mps_matvec_policy.h"
#import "aljabr_metal_matvec_dispatch.h"
#import "aljabr_metal_matvec_tuning.h"
#import "aljabr_metal_pipelines.h"

#define env_truthy aljabr_metal_env_truthy
#define env_int_or_default aljabr_metal_env_int_or_default
#define env_float_or_default aljabr_metal_env_float_or_default
#define monotonic_nanos aljabr_metal_monotonic_nanos
...
static int aljabr_metal_matvec_tb_half_custom(void* C,
                           const void* A,
                           const void* B,
                           int K, int N) {
    ...
    id<MTLComputePipelineState> pipeline =
            threads == ALJABR_MATVEC_THREADS_128 ? g_matvec_half_128_pipeline : g_matvec_half_pipeline;
    return dispatch_matvec_tb_half(C, A, B, K, N, pipeline, threads);
}

int aljabr_metal_matvec_tb_half(void* C,
                           const void* A,
                           const void* B,
                           int K, int N) {
    if (!g_initialized) return -1;
    ...
    if (aljabr_metal_mps_matvec_should_try(K, N)) {
        @autoreleasepool {
            NSString* shapeKey = mps_matvec_shape_key(K, N);
            BOOL validateEveryCall = aljabr_metal_mps_matvec_validate_every_call();
            BOOL autotune = aljabr_metal_mps_matvec_autotune_enabled_for_output(N);
            AljabrMetalMpsMatvecShapeState shapeState =
                    aljabr_metal_mps_matvec_shape_state(shapeKey, validateEveryCall, autotune);
            if (!shapeState.failed && !shapeState.custom_preferred) {
                uint64_t mpsStart = autotune ? monotonic_nanos() : 0;
                uint16_t* halfA = aljabr_metal_ensure_half_input_scratch((size_t)K);
                uint16_t* halfC = aljabr_metal_ensure_half_output_scratch((size_t)N);
                    ...
                    @try {
                        ...
                        if ([cmd status] == MTLCommandBufferStatusCompleted) {
                            ...
                            if (shapeState.validated
                                ...
                                if (!shapeState.validated && !validateEveryCall && !env_truthy("ALJABR_METAL_MPS_MATVEC_SKIP_VALIDATE")) {
                                    aljabr_metal_mps_matvec_mark_validated(shapeKey);
                                    if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                                        fprintf(stderr,
                                                "[aljabr-metal] MPS matvec validated shape K=%d N=%d\n",
                                                K, N);
                                    }
                                }
                                if (autotune && !shapeState.mps_preferred) {
                                    uint64_t customStart = monotonic_nanos();
                                    int customRc = aljabr_metal_matvec_tb_half_custom(C, A, B, K, N);
                                    uint64_t customNanos = monotonic_nanos() - customStart;
                                    if (customRc == 0) {
                                        float margin = env_float_or_default(
                                                "ALJABR_METAL_MPS_MATVEC_AUTOTUNE_MARGIN", 0.05f);
                                        if (margin < 0.0f) margin = 0.0f;
                                        BOOL preferMps = (double)mpsNanos < ((double)customNanos * (1.0 - (double)margin));
                                        aljabr_metal_mps_matvec_record_autotune_preference(shapeKey, preferMps);
                                        if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                                            fprintf(stderr,
                                                    "[aljabr-metal] MPS matvec autotune shape K=%d N=%d mps=%.3fms custom=%.3fms selected=%s\n",
                                                    K, N,
                                    ...
                                    aljabr_metal_mps_matvec_mark_mps_preferred(shapeKey);
                                }
                                return 0;
                            }
                            aljabr_metal_mps_matvec_mark_failed(shapeKey);
                            if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                                fprintf(stderr,
                                        "[aljabr-metal] disabling MPS matvec for shape K=%d N=%d; falling back to custom Metal reduction\n",
                                        K, N);
                            }
                        } else {
                            aljabr_metal_mps_matvec_mark_disable_after_failure();
                            if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                                NSString* message = cmd.error.localizedDescription;
                                fprintf(stderr,
                                        "[aljabr-metal] MPS matvec command failed (%s); falling back to custom Metal reduction\n",
                        ...
                        if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                            ...
                            fprintf(stderr,
                                    "[aljabr-metal] MPS matvec threw %s; falling back to custom Metal reduction\n",
...
int aljabr_metal_matvec_tb_half_mps(void* C,
                           const void* A,
                           const void* B,
                           int K, int N) {
    if (!g_initialized) return -1;
    ...
    @synchronized(aljabr_metal_mps_matvec_policy_lock()) {
        AljabrMetalMpsMatvecOverrideSnapshot snapshot = aljabr_metal_mps_matvec_force_shape(K, N);
        if (!env_truthy("ALJABR_METAL_VALIDATE_LOGITS_MPS_MATVEC")
                && !env_truthy("ALJABR_METAL_MPS_MATVEC_VALIDATE_EVERY_CALL")) {
            aljabr_metal_mps_matvec_mark_validated(mps_matvec_shape_key(K, N));
        }
        int rc = aljabr_metal_matvec_tb_half(C, A, B, K, N);
...
int aljabr_metal_matvec_t_half(void* C,
    ...
    if (threads == 0 && matvec_autotune_enabled(K, N, can128) && g_matvec_t_half_pipeline != nil) {
        ...
        int rc128 = dispatch_matvec_t_half(C, A, B, K, N,
                g_matvec_t_half_128_pipeline, ALJABR_MATVEC_THREADS_128);
        uint64_t nanos128 = monotonic_nanos() - start128;
        uint64_t start256 = monotonic_nanos();
        int rc256 = dispatch_matvec_t_half(C, A, B, K, N,
                g_matvec_t_half_pipeline, ALJABR_MATVEC_THREADS_256);
        uint64_t nanos256 = monotonic_nanos() - start256;
        if (rc128 == 0 && rc256 == 0) {
            threads = matvec_autotune_prefers_128(nanos128, nanos256)
                    ? ALJABR_MATVEC_THREADS_128
    ...
    id<MTLComputePipelineState> pipeline =
            threads == ALJABR_MATVEC_THREADS_128 ? g_matvec_t_half_128_pipeline : g_matvec_t_half_pipeline;
    return dispatch_matvec_t_half(C, A, B, K, N, pipeline, threads);
}

static int aljabr_metal_matvec_tb_bf16_custom(void* C,
                           const void* A,
                           const void* B,
                           int K, int N) {
    if (!g_initialized) return -1;
    ...
    id<MTLComputePipelineState> pipeline =
            threads == ALJABR_MATVEC_THREADS_128 ? g_matvec_bf16_128_pipeline : g_matvec_bf16_pipeline;
    return dispatch_matvec_tb_half(C, A, B, K, N, pipeline, threads);
}

int aljabr_metal_matvec_tb_bf16(void* C,
                           const void* A,
                           const void* B,
                           int K, int N) {
    if (!g_initialized) return -1;
    ...
    if (should_use_bf16_matvec_x8(K, N) && g_matvec_bf16_x8_pipeline != nil) {
        return dispatch_matvec_tb_half_x8(C, A, B, K, N,
                g_matvec_bf16_x8_pipeline, ALJABR_MATVEC_THREADS_256);
    }
    if (should_use_simdgroup_reduction()
        ...
        return dispatch_matvec_tb_half_x4(C, A, B, K, N,
                g_matvec_bf16_x4_simd_pipeline, ALJABR_MATVEC_THREADS_256);
    }
    if (should_use_bf16_matvec_x4(K, N) && g_matvec_bf16_x4_pipeline != nil) {
        return dispatch_matvec_tb_half_x4(C, A, B, K, N,
                g_matvec_bf16_x4_pipeline, ALJABR_MATVEC_THREADS_256);
    }

    if (aljabr_metal_mps_bf16_matvec_should_try(K, N)) {
        @autoreleasepool {
            ...
            if (!shapeState.failed && !shapeState.custom_preferred) {
                ...
                if (bf16MpsAvailable) {
                    uint64_t mpsStart = autotune ? monotonic_nanos() : 0;
                    uint16_t* bf16A = aljabr_metal_ensure_half_input_scratch((size_t)K);
                    uint16_t* bf16C = aljabr_metal_ensure_half_output_scratch((size_t)N);
                    ...
                    if (bufC != nil && bufA != nil && bufB != nil) {
                        @try {
                            ...
                            if ([cmd status] == MTLCommandBufferStatusCompleted) {
                                ...
                                if (shapeState.validated
                                    ...
                                    if (!shapeState.validated && !validateEveryCall && !env_truthy("ALJABR_METAL_MPS_MATVEC_SKIP_VALIDATE")) {
                                        ...
                                        if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                                            fprintf(stderr,
                                                    "[aljabr-metal] MPS BF16 matvec validated shape K=%d N=%d\n",
                                                    K, N);
                                        }
                                    }
                                    if (autotune && !shapeState.mps_preferred) {
                                        uint64_t customStart = monotonic_nanos();
                                        int customRc = aljabr_metal_matvec_tb_bf16_custom(C, A, B, K, N);
                                        uint64_t customNanos = monotonic_nanos() - customStart;
                                        if (customRc == 0) {
                                            float margin = env_float_or_default(
                                                    "ALJABR_METAL_MPS_MATVEC_AUTOTUNE_MARGIN", 0.05f);
                                            ...
                                            if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                                                fprintf(stderr,
                                                        "[aljabr-metal] MPS BF16 matvec autotune shape K=%d N=%d mps=%.3fms custom=%.3fms selected=%s\n",
                                                        K, N,
                                                        (double)mpsNanos / 1000000.0,
                                                        (double)customNanos / 1000000.0,
                                                        preferMps ? "mps" : "custom");
                                ...
                                if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                                    fprintf(stderr,
                                            "[aljabr-metal] disabling MPS BF16 matvec for shape K=%d N=%d; falling back to custom Metal reduction\n",
                                            K, N);
                                }
                            } else {
                                aljabr_metal_mps_matvec_mark_disable_after_failure();
                                if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                                    NSString* message = cmd.error.localizedDescription;
                                    fprintf(stderr,
                                            "[aljabr-metal] MPS BF16 matvec command failed (%s); falling back to custom Metal reduction\n",
                                            message != nil ? [message UTF8String] : "unknown error");
                                }
                            }
                        } @catch (NSException* ex) {
                            ...
                            if (env_truthy("ALJABR_METAL_MPS_MATVEC_DEBUG")) {
                                NSString* reason = [ex reason];
                                fprintf(stderr,
                                        "[aljabr-metal] MPS BF16 matvec threw %s; falling back to custom Metal reduction\n",
                                        reason != nil ? [reason UTF8String] : "unknown exception");
                            }
                        }
                    }
    ...
    if (threads == 0 && matvec_autotune_enabled(K, N, can128) && g_matvec_bf16_pipeline != nil) {
        uint64_t start128 = monotonic_nanos();
        int rc128 = dispatch_matvec_tb_half(C, A, B, K, N,
                g_matvec_bf16_128_pipeline, ALJABR_MATVEC_THREADS_128);
        uint64_t nanos128 = monotonic_nanos() - start128;
        uint64_t start256 = monotonic_nanos();
        int rc256 = dispatch_matvec_tb_half(C, A, B, K, N,
                g_matvec_bf16_pipeline, ALJABR_MATVEC_THREADS_256);
        uint64_t nanos256 = monotonic_nanos() - start256;
        if (rc128 == 0 && rc256 == 0) {
            threads = matvec_autotune_prefers_128(nanos128, nanos256)
                    ? ALJABR_MATVEC_THREADS_128
...
int aljabr_metal_matvec_tb_half_pair(void* C0, void* C1,
                           const void* A,
    ...
    if (threads == 0 && matvec_autotune_enabled(K, N, can128) && g_matvec_half_pair_pipeline != nil) {
        ...
        int rc128 = dispatch_matvec_tb_half_pair(C0, C1, A, B0, B1, K, N,
                g_matvec_half_pair_128_pipeline, ALJABR_MATVEC_THREADS_128);
        uint64_t nanos128 = monotonic_nanos() - start128;
        uint64_t start256 = monotonic_nanos();
        int rc256 = dispatch_matvec_tb_half_pair(C0, C1, A, B0, B1, K, N,
                g_matvec_half_pair_pipeline, ALJABR_MATVEC_THREADS_256);
        uint64_t nanos256 = monotonic_nanos() - start256;
        if (rc128 == 0 && rc256 == 0) {
            threads = matvec_autotune_prefers_128(nanos128, nanos256)
                    ? ALJABR_MATVEC_THREADS_128
    ...
    id<MTLComputePipelineState> pipeline =
            threads == ALJABR_MATVEC_THREADS_128 ? g_matvec_half_pair_128_pipeline : g_matvec_half_pair_pipeline;
    return dispatch_matvec_tb_half_pair(C0, C1, A, B0, B1, K, N, pipeline, threads);
}

int aljabr_metal_matvec_tb_bf16_pair(void* C0, void* C1,
                           const void* A,
                           const void* B0, const void* B1,
                           int K, int N) {
    if (!g_initialized) return -1;
    if (K <= 0 || N <= 0) return -2;
    BOOL usePairX4 = !env_truthy("ALJABR_METAL_DISABLE_BF16_PAIR_X4")
            && g_matvec_bf16_pair_x4_pipeline != nil
            && (env_truthy("ALJABR_METAL_ENABLE_BF16_PAIR_X4")
                    || should_use_bf16_matvec_x4(K, N));
    if (usePairX4) {
        ...
        return dispatch_matvec_tb_half_pair(C0, C1, A, B0, B1, K, N,
                pipeline, ALJABR_MATVEC_THREADS_256);
    }
    if (should_use_bf16_pair_simd_reduction(K, N) && g_matvec_bf16_pair_simd_pipeline != nil) {
        return dispatch_matvec_tb_half_pair(C0, C1, A, B0, B1, K, N,
                g_matvec_bf16_pair_simd_pipeline, ALJABR_MATVEC_THREADS_256);
    }
    ...
    if (threads == 0 && matvec_autotune_enabled(K, N, can128) && g_matvec_bf16_pair_pipeline != nil) {
        uint64_t start128 = monotonic_nanos();
        int rc128 = dispatch_matvec_tb_half_pair(C0, C1, A, B0, B1, K, N,
                g_matvec_bf16_pair_128_pipeline, ALJABR_MATVEC_THREADS_128);
        uint64_t nanos128 = monotonic_nanos() - start128;
        uint64_t start256 = monotonic_nanos();
        int rc256 = dispatch_matvec_tb_half_pair(C0, C1, A, B0, B1, K, N,
                g_matvec_bf16_pair_pipeline, ALJABR_MATVEC_THREADS_256);
        uint64_t nanos256 = monotonic_nanos() - start256;
        if (rc128 == 0 && rc256 == 0) {
            threads = matvec_autotune_prefers_128(nanos128, nanos256)
                    ? ALJABR_MATVEC_THREADS_128
    ...
    id<MTLComputePipelineState> pipeline =
            threads == ALJABR_MATVEC_THREADS_128 ? g_matvec_bf16_pair_128_pipeline : g_matvec_bf16_pair_pipeline;
    return dispatch_matvec_tb_half_pair(C0, C1, A, B0, B1, K, N, pipeline, threads);
}

int aljabr_metal_matvec_tb_half_triple_mixed(void* C0, void* C1, void* C2,
                           const void* A,
    ...
    if (threads == 0 && matvec_autotune_enabled(K, total, can128) && g_matvec_half_triple_mixed_pipeline != nil) {
        ...
        int rc128 = dispatch_matvec_tb_half_triple_mixed(C0, C1, C2, A, B0, B1, B2, K, N0, N1, N2,
                g_matvec_half_triple_mixed_128_pipeline, ALJABR_MATVEC_THREADS_128);
        uint64_t nanos128 = monotonic_nanos() - start128;
        uint64_t start256 = monotonic_nanos();
        int rc256 = dispatch_matvec_tb_half_triple_mixed(C0, C1, C2, A, B0, B1, B2, K, N0, N1, N2,
                g_matvec_half_triple_mixed_pipeline, ALJABR_MATVEC_THREADS_256);
        uint64_t nanos256 = monotonic_nanos() - start256;
        if (rc128 == 0 && rc256 == 0) {
            threads = matvec_autotune_prefers_128(nanos128, nanos256)
                    ? ALJABR_MATVEC_THREADS_128
    ...
    id<MTLComputePipelineState> pipeline = threads == ALJABR_MATVEC_THREADS_128
...
int aljabr_metal_matvec_tb_bf16_triple_mixed(void* C0, void* C1, void* C2,
                           const void* A,
                           const void* B0, const void* B1, const void* B2,
                           int K, int N0, int N1, int N2) {
    if (!g_initialized) return -1;
    ...
    if (should_use_bf16_matvec_x4(K, total) && g_matvec_bf16_triple_mixed_x4_pipeline != nil) {
        return dispatch_matvec_tb_half_triple_mixed(C0, C1, C2, A, B0, B1, B2,
                K, N0, N1, N2, g_matvec_bf16_triple_mixed_x4_pipeline, ALJABR_MATVEC_THREADS_256);
    }
    ...
    if (threads == 0 && matvec_autotune_enabled(K, total, can128) && g_matvec_bf16_triple_mixed_pipeline != nil) {
        uint64_t start128 = monotonic_nanos();
        int rc128 = dispatch_matvec_tb_half_triple_mixed(C0, C1, C2, A, B0, B1, B2, K, N0, N1, N2,
                g_matvec_bf16_triple_mixed_128_pipeline, ALJABR_MATVEC_THREADS_128);
        uint64_t nanos128 = monotonic_nanos() - start128;
        uint64_t start256 = monotonic_nanos();
        int rc256 = dispatch_matvec_tb_half_triple_mixed(C0, C1, C2, A, B0, B1, B2, K, N0, N1, N2,
                g_matvec_bf16_triple_mixed_pipeline, ALJABR_MATVEC_THREADS_256);
        uint64_t nanos256 = monotonic_nanos() - start256;
        if (rc128 == 0 && rc256 == 0) {
            threads = matvec_autotune_prefers_128(nanos128, nanos256)
                    ? ALJABR_MATVEC_THREADS_128
