/**
 * aljabr_metal_mps_matmul.m — public MPS matrix multiplication kernels.
 */

#import "aljabr_metal_mps_matmul.h"
...
void aljabr_metal_mps_matmul_configure_from_env(void) {
    const char* threshold_env = getenv("ALJABR_METAL_SMALL_MATMUL_CPU_THRESHOLD");
...
int aljabr_metal_matmul(void* C, const void* A, const void* B,
                         int M, int K, int N,
                         float alpha, float beta) {
    if (!g_initialized) return -1;

    if (M * N < g_small_matmul_cpu_threshold) {
        return aljabr_metal_cpu_matmul_nn(C, A, B, M, K, N, alpha, beta);
    }
...
int aljabr_metal_matmul_tb(void* C, const void* A, const void* B,
                           int M, int K, int N,
                           float alpha, float beta) {
    if (!g_initialized) return -1;
...
int aljabr_metal_matmul_tb_half_pair(void* C0, void* C1,
                           const void* A,
...
int aljabr_metal_matmul_tb_half_pair_mixed(void* C0, void* C1,
                           const void* A,
                           const void* B0, const void* B1,
...
int aljabr_metal_matmul_tb_half_triple_mixed(void* C0, void* C1, void* C2,
                           const void* A,
