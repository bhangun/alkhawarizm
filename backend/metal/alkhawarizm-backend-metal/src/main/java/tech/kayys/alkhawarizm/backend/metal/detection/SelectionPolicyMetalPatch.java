package tech.kayys.alkhawarizm.metal.detection;
...
import tech.kayys.alkhawarizm.engine.routing.policy.SelectionPolicy;
...
import tech.kayys.alkhawarizm.runner.RunnerCandidate;
import tech.kayys.alkhawarizm.model.core.RunnerMetrics;
import tech.kayys.alkhawarizm.model.core.HardwareDetector;
 ...
 * when the {@code alkhawarizm-ext-metal} module is on the classpath.
 *
 * <p>
    ...
    public List<RunnerCandidate> rankRunners(
        ...
        if (mode == MetalRunnerMode.DISABLED) {
            ...
            if (ranked.size() < configuredRunners.size()) {
                LOG.info("[MetalPatch] Metal runners disabled via alkhawarizm.runners.metal.mode=disabled");
            }
            return ranked;
        }
