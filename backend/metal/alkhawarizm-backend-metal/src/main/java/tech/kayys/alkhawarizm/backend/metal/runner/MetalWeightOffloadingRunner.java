package tech.kayys.alkhawarizm.metal.runner;
...
import tech.kayys.alkhawarizm.metal.detection.MetalCapabilities;
 ...
 * <p>
 * On CUDA hardware, "CPU offloading" means weight tensors live in host RAM and
 * must be explicitly copied to GPU VRAM before each layer executes. That copy
 * costs
 * PCIe bandwidth (~16 GB/s) and latency.
 *
 * <p>
 * On Apple Silicon with its <b>Unified Memory Architecture</b>, CPU and GPU
 * share
 * the same physical DRAM. A memory-mapped file segment
 * ({@link FileChannel#map}) or
 ...
 * <p>
 * Rather than streaming layers one-by-one like the CUDA offload runner, this
 * runner <em>pins</em> the most-frequently-accessed layers in the "hot" region
 * of
 ...
 * 
 * <pre>
 *   alkhawarizm.runners.metal-offload.enabled=false
 *   alkhawarizm.runners.metal.mode=auto  # auto|standard|offload|force|disabled
 *   alkhawarizm.runners.metal-offload.library-path=~/.alkhawarizm/libs/libalkhawarizm_metal.dylib
 *   alkhawarizm.runners.metal-offload.hot-layers=8
 *   alkhawarizm.runners.metal-offload.pin-refresh-interval-ms=10000
 *   alkhawarizm.runners.metal-offload.num-layers=32
 *   alkhawarizm.runners.metal-offload.model-dim=4096
 *   alkhawarizm.runners.metal-offload.head-dim=128
 *   alkhawarizm.runners.metal-offload.num-heads=32
 *   alkhawarizm.runners.metal-offload.num-heads-kv=8
 *   alkhawarizm.runners.metal-offload.ffn-dim=14336
 * </pre>
 */
@ApplicationScoped
public class MetalWeightOffloadingRunner extends AbstractAljabrRunner {
    ...
    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.enabled", defaultValue = "false")
    boolean enabled;

    @ConfigProperty(name = "alkhawarizm.runners.metal.mode", defaultValue = "auto")
    String metalMode;

    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.library-path", defaultValue = "~/.alkhawarizm/libs/libalkhawarizm_metal.dylib")
    String libraryPath;

    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.hot-layers", defaultValue = "8")
    int hotLayers;

    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.pin-refresh-interval-ms", defaultValue = "10000")
    long pinRefreshIntervalMs;

    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.num-layers", defaultValue = "32")
    int numLayers;

    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.model-dim", defaultValue = "4096")
    int modelDim;

    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.head-dim", defaultValue = "128")
    int headDim;

    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.num-heads", defaultValue = "32")
    int numHeads;

    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.num-heads-kv", defaultValue = "8")
    int numHeadsKv;

    @ConfigProperty(name = "alkhawarizm.runners.metal-offload.ffn-dim", defaultValue = "14336")
    int ffnDim;

    @Inject
    PagedKVCacheManager kvCacheManager;
    @Inject
    AppleSiliconDetector detector;
    ...
    {
        layerAccessCount = new long[256];
    } // pre-size; resized in initialize()
    ...
    public void initialize(ModelManifest modelManifest, RunnerConfiguration config)
        ...
        if (mode == MetalRunnerMode.DISABLED)
            throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "MetalWeightOffload runner disabled (alkhawarizm.runners.metal.mode=disabled)");
        if (mode == MetalRunnerMode.STANDARD)
            throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "MetalWeightOffload runner disabled (alkhawarizm.runners.metal.mode=standard)");
    ...
    private void pinRefreshLoop() {
        while (!Thread.currentThread().isInterrupted()) {
            try {
                ...
                for (int i = 0; i < Math.min(numLayers, counts.length); i++)
                    sorted.add(i);
    ...
    private float[] readLogits(MemorySegment seg, int vocab) {
        ...
        for (int i = 0; i < n; i++)
            out[i] = seg.getAtIndex(ValueLayout.JAVA_FLOAT, i);
        return out;
    }
    ...
    private MemorySegment packInts(Arena arena, int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++)
            seg.setAtIndex(ValueLayout.JAVA_INT, i, arr[i]);
        return seg;
    }
    ...
    public boolean health() {
        return initialized && metal != null;
    }

    @Override
    public void close() {
        initialized = false;
        if (pinRefreshThread != null)
            pinRefreshThread.interrupt();
