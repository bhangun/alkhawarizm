 ...
 * <p>
 * Runs transformer inference <b>in-process</b> on Apple Silicon (M1/M2/M3/M4)
 * using two FFM binding classes:
 * <ul>
 * <li>{@link MetalFlashAttentionBinding} — FA4-equivalent fused attention via
 * {@code alkhawarizm_metal_fa4_attention} (MPSGraph SDPA on macOS 14+).</li>
 ...
 * <p>
 * FA4 targets NVIDIA Blackwell. On Apple Silicon the same algorithmic goal —
 ...
 * </tr>
 * <tr>
 * <td>TMEM tile accumulator</td>
 * <td>GPU tile cache (on-chip SRAM)</td>
 * </tr>
 * <tr>
 ...
 * </tr>
 * <tr>
 * <td>Software exp() on FMA</td>
 * <td>Apple M-series FP16 exp intrinsic</td>
 * </tr>
 * <tr>
 * <td>Fused single-pass kernel</td>
 * <td>MPSGraph SDPA (macOS 14+ M3/M4)</td>
 * </tr>
 * <tr>
 * <td>No full attn matrix on DRAM</td>
 * <td>Same guarantee via MPS tiling</td>
 * </tr>
 * </table>
 ...
 * <p>
 * On Apple Silicon all {@link MemorySegment} allocations
 * ({@link Arena#ofShared()} or
 ...
 * 
 * <pre>
 *   alkhawarizm.runners.metal.enabled=false
 *   alkhawarizm.runners.metal.mode=auto  # auto|standard|offload|force|disabled
 *   alkhawarizm.runners.metal.library-path=~/.alkhawarizm/libs/libalkhawarizm_metal.dylib
 *   alkhawarizm.runners.metal.num-layers=32
 *   alkhawarizm.runners.metal.num-heads=32
 *   alkhawarizm.runners.metal.num-heads-kv=8
 *   alkhawarizm.runners.metal.head-dim=128
 *   alkhawarizm.runners.metal.model-dim=4096
 *   alkhawarizm.runners.metal.ffn-dim=14336
 *   alkhawarizm.runners.metal.vocab-size=32000
 * </pre>
 *
...
public class MetalRunner extends AbstractAljabrRunner {
    ...
    @ConfigProperty(name = "alkhawarizm.runners.metal.enabled", defaultValue = "false")
    boolean enabled;

    @ConfigProperty(name = "alkhawarizm.runners.metal.mode", defaultValue = "auto")
    String metalMode;

    @ConfigProperty(name = "alkhawarizm.runners.metal.library-path", defaultValue = "~/.alkhawarizm/libs/libalkhawarizm_metal.dylib")
    String libraryPath;

    @ConfigProperty(name = "alkhawarizm.runners.metal.num-layers", defaultValue = "32")
    int numLayers;

    @ConfigProperty(name = "alkhawarizm.runners.metal.num-heads", defaultValue = "32")
    int numHeads;

    @ConfigProperty(name = "alkhawarizm.runners.metal.num-heads-kv", defaultValue = "8")
    int numHeadsKv;

    @ConfigProperty(name = "alkhawarizm.runners.metal.head-dim", defaultValue = "128")
    int headDim;

    @ConfigProperty(name = "alkhawarizm.runners.metal.model-dim", defaultValue = "4096")
    int modelDim;

    @ConfigProperty(name = "alkhawarizm.runners.metal.ffn-dim", defaultValue = "14336")
    int ffnDim;

    @ConfigProperty(name = "alkhawarizm.runners.metal.vocab-size", defaultValue = "32000")
    int vocabSize;
    ...
    public RunnerMetadata metadata() {
        String attnPath = metalFa4 != null && metalFa4.isSdpaAvailable()
                ? "MPSGraph-SDPA-FA4-equiv"
                : "MPS-separate-matmuls";
    ...
    public void initialize(ModelManifest modelManifest, RunnerConfiguration config)
        ...
        if (!enabled)
            throw new RunnerInitializationException(
                    ...
                    "Metal runner disabled (alkhawarizm.runners.metal.enabled=false)");
        if (mode == MetalRunnerMode.DISABLED)
            throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "Metal runner disabled (alkhawarizm.runners.metal.mode=disabled)");
        if (mode == MetalRunnerMode.OFFLOAD)
            throw new RunnerInitializationException(
                    ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                    "Metal runner disabled (alkhawarizm.runners.metal.mode=offload)");
        ...
        if (metal.isNativeAvailable()) {
            int err = metal.init(); // alkhawarizm_metal_init() — MTLCreateSystemDefaultDevice
            if (err != 0)
                throw new RunnerInitializationException(
                        ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                        "alkhawarizm_metal_init() returned " + err);
            if (!metal.isRuntimeActive())
                throw new RunnerInitializationException(
                        ErrorCode.INIT_NATIVE_LIBRARY_FAILED.name(),
                        "alkhawarizm_metal_init() fell back to CPU; native Metal runtime is not active");
        }
     ...
     * <li>FA4-equivalent paged attention —
     * {@link MetalFlashAttentionBinding#fa4Attention}
     ...
     *
     * <p>
     * Attention uses {@link MetalFlashAttentionBinding#fa4Attention} with
    ...
    private float[] runForwardPass(Arena arena,
        ...
        for (int layer = 0; layer < numLayers; layer++) {
            ...
            // [normW(D), W_qkv(D×qkvDim), W_o(qDim×D), ffnNormW(D), W_gate(D×F), W_up(D×F),
            // W_down(F×D)]
            long off = 0L;
