/**
 * alkhawarizm_metal_mps_matvec_policy.m — MPS matvec validation/autotune
 * policy.
 */
...
static BOOL base_should_try(int K, int N) {
  ...
  int maxOutput = maxOutputOverride >= 0
                      ? maxOutputOverride
                      : alkhawarizm_metal_env_int_or_default(
                            "ALKHAWARIZM_METAL_MPS_MATVEC_MAX_OUTPUT", 640);
  int maxInner = maxInnerOverride >= 0
                     ? maxInnerOverride
                     : alkhawarizm_metal_env_int_or_default(
                           "ALKHAWARIZM_METAL_MPS_MATVEC_MAX_INNER", 2048);

  return !disabledAfterFailure &&
         (enabled || alkhawarizm_metal_env_truthy(
                         "ALKHAWARIZM_METAL_ENABLE_MPS_MATVEC")) &&
         (maxInner <= 0 || K <= maxInner) &&
         (maxOutput <= 0 || N <= maxOutput) &&
         !alkhawarizm_metal_env_truthy("ALKHAWARIZM_METAL_DISABLE_MPS_MATVEC");
}
...
BOOL alkhawarizm_metal_mps_bf16_matvec_should_try(int K, int N) {
  return base_should_try(K, N)
         ...
         && alkhawarizm_metal_env_truthy(
                "ALKHAWARIZM_METAL_UNSAFE_ENABLE_MPS_BF16_MATVEC") &&
         !alkhawarizm_metal_env_truthy(
             "ALKHAWARIZM_METAL_DISABLE_MPS_BF16_MATVEC");
}
...
BOOL alkhawarizm_metal_mps_matvec_autotune_enabled_for_output(int N) {
  ...
  int maxOutput =
      maxOutputOverride >= 0
          ? maxOutputOverride
          : alkhawarizm_metal_env_int_or_default(
                "ALKHAWARIZM_METAL_MPS_MATVEC_AUTOTUNE_MAX_OUTPUT", 1024);
  return (enabled || alkhawarizm_metal_env_truthy(
                         "ALKHAWARIZM_METAL_MPS_MATVEC_AUTOTUNE")) &&
         (maxOutput <= 0 || N <= maxOutput);
}
...
AljabrMetalMpsMatvecShapeState alkhawarizm_metal_mps_matvec_shape_state(
  ...
  @synchronized(alkhawarizm_metal_mps_matvec_policy_lock()) {
    state.failed =
        g_failed_shapes != nil && [g_failed_shapes containsObject:shapeKey];
    state.validated = !validateEveryCall && g_validated_shapes != nil &&
                      [g_validated_shapes containsObject:shapeKey];
    state.mps_preferred = autotune && g_mps_preferred_shapes != nil &&
                          [g_mps_preferred_shapes containsObject:shapeKey];
    state.custom_preferred =
        autotune && g_custom_preferred_shapes != nil &&
