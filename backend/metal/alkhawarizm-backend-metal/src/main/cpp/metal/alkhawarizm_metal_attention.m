/**
 * alkhawarizm_metal_attention.m — paged attention bridge implementation.
 */
...
static int alkhawarizm_metal_decode_attention_gpu(
    void *out, const void *Q, const void *K_cache, const void *V_cache,
    const int *block_table, const int *context_lens, int B, int H, int H_kv,
    int D, int block_size, int max_blocks, float scale, int is_causal,
    int query_start_pos, int sliding_window, float soft_cap) {
  id<MTLComputePipelineState> pipeline =
      alkhawarizm_metal_pipelines()->decode_attention;
  if (!g_initialized || pipeline == nil)
    return -3;
  if (alkhawarizm_metal_env_truthy(
          "ALKHAWARIZM_METAL_DISABLE_DECODE_ATTENTION_KERNEL"))
    return -9;
  if (B <= 0 || H <= 0 || H_kv <= 0 || D <= 0 || block_size <= 0 ||
      max_blocks <= 0)
    return -2;
  if (H % H_kv != 0)
    return -2;
  ...
  int max_context = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_DECODE_ATTENTION_MAX_CONTEXT", 4096);
  ...
  for (int b = 0; b < B; b++) {
    ...
    if (max_pos >= ctx_len)
      max_pos = ctx_len - 1;
    int min_pos = sliding_window > 0
                      ? (effective_query_start_pos - sliding_window + 1)
                      : 0;
    if (min_pos < 0)
      min_pos = 0;
  ...
  @autoreleasepool {
    ...
    size_t kv_bytes =
        (size_t)(max_phys + 1) * H_kv * block_size * D * sizeof(float);
    ...
    id<MTLBuffer> bufBlockTable =
        wrap_ptr((void *)block_table, block_table_bytes);
    id<MTLBuffer> bufContextLens =
        wrap_ptr((void *)context_lens, context_lens_bytes);
...
static inline void resolve_attention_bounds(int ctx_len, int is_causal,
                                            int query_start_pos, int query_idx,
                                            int sliding_window, int *min_pos,
                                            int *max_pos) {
...
static int alkhawarizm_metal_attention_impl(
    void *out, const void *Q, const void *K_cache, const void *V_cache,
  ...
  if (H_kv <= 0 || H % H_kv != 0)
    return -1;
  if (B <= 0 || T <= 0 || H <= 0 || D <= 0 || block_size <= 0 ||
      max_blocks <= 0)
    return -2;

  int gqa_group = H / H_kv;
  if (T == 1) {
    int gpu_rc = alkhawarizm_metal_decode_attention_gpu(
        out, Q, K_cache, V_cache, block_table, context_lens, B, H, H_kv, D,
        block_size, max_blocks, scale, is_causal, query_start_pos,
        sliding_window, soft_cap);
    if (gpu_rc == 0) {
      return 0;
    }
    if (gpu_rc != -9 && alkhawarizm_metal_env_truthy(
                            "ALKHAWARIZM_METAL_DECODE_ATTENTION_DEBUG")) {
      fprintf(stderr,
              "[alkhawarizm-metal] decode attention GPU kernel unavailable "
              "rc=%d; falling back to CPU attention bridge\n",
              gpu_rc);
    }
  }
  BOOL use_decode_sgemv =
      (T == 1) && !alkhawarizm_metal_env_truthy(
                      "ALKHAWARIZM_METAL_DISABLE_ATTENTION_SGEMV");
  ...
  for (int b = 0; b < B; b++) {
    ...
    size_t scratch_elements =
        score_elements > kv_elements ? score_elements : kv_elements;
    if (!alkhawarizm_metal_ensure_attention_scratch(scratch_elements, &kPtr,
                                                    &vPtr, &scoreBuf)) {
      return -4;
    }
    ...
    if (block_table != NULL) {
      for (int blk = 0; blk < num_blocks; blk++) {
        ...
        if (phys < 0)
          return -2;
        int tokens_in_blk =
            (blk == num_blocks - 1) ? (ctx_len - blk * block_size) : block_size;

        for (int h = 0; h < H_kv; h++) {
          const float *src_k =
              kcache + ((size_t)phys * H_kv + h) * block_size * D;
          const float *src_v =
              vcache + ((size_t)phys * H_kv + h) * block_size * D;
    ...
    for (int h = 0; h < H; h++) {
      ...
      for (int t = 0; t < T; t++) {
        ...
        resolve_attention_bounds(ctx_len, is_causal, effective_query_start_pos,
                                 t, sliding_window, &min_pos, &max_pos);
        ...
        if (use_decode_sgemv) {
          ...
          if (block_table != NULL) {
            ...
            kBase =
                kcache + (((size_t)b * ctx_len + min_pos) * H_kv + kv_h) * D;
          }
          alkhawarizm_metal_cpu_matvec_rows(row + min_pos, kBase, qh, valid_len,
                                            D, lda, scale, 0.0f);
          ...
          if (block_table != NULL) {
            ...
            vBase =
                vcache + (((size_t)b * ctx_len + min_pos) * H_kv + kv_h) * D;
          }
          alkhawarizm_metal_cpu_matvec_cols(oh, vBase, row + min_pos, valid_len,
                                            D, lda, 1.0f, 0.0f);
          continue;
        }
...
int alkhawarizm_metal_attention(void *out, const void *Q, const void *K_cache,
  ...
  return alkhawarizm_metal_attention_impl(
      out, Q, K_cache, V_cache, block_table, context_lens, B, T, H, H, D,
      block_size, max_blocks, scale, is_causal, 0, 0, soft_cap);
}
...
int alkhawarizm_metal_attention_windowed(
  ...
  return alkhawarizm_metal_attention_impl(
      out, Q, K_cache, V_cache, block_table, context_lens, B, T, H, H, D,
      block_size, max_blocks, scale, is_causal, query_start_pos, sliding_window,
      soft_cap);
}
...
int alkhawarizm_metal_attention_gqa(void *out, const void *Q,
                                    ...
                                    float soft_cap) {
  return alkhawarizm_metal_attention_impl(
      out, Q, K_cache, V_cache, block_table, context_lens, B, T, H, H_kv, D,
      block_size, max_blocks, scale, is_causal, 0, 0, soft_cap);
}
...
int alkhawarizm_metal_flash_attention(void *out, const void *q, const void *k,
  ...
  id<MTLComputePipelineState> pipeline =
      alkhawarizm_metal_pipelines()->flash_attention;
  if (!g_initialized || pipeline == nil)
    return -3;
