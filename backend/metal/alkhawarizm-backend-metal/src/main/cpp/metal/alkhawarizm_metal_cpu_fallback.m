/**
 * alkhawarizm_metal_cpu_fallback.m — CPU fallback kernels for the Metal bridge.
 */
...
static const int ALKHAWARIZM_CPU_FFN_VECTOR_MIN_ELEMENTS = 2048;

static int ranges_overlap(const void *a, size_t a_bytes, const void *b,
                          size_t b_bytes) {
...
static int ensure_fallback_scratch(size_t elements, float **scratch0,
                                   float **scratch1) {
  if (scratch0 != NULL)
    *scratch0 = NULL;
  if (scratch1 != NULL)
  ...
  if (scratch0 != NULL)
    *scratch0 = tl_fallback_scratch0;
  if (scratch1 != NULL)
    *scratch1 = tl_fallback_scratch1;
  return 1;
}
...
int alkhawarizm_metal_cpu_add(void *C, const void *A, const void *B, int N) {
  ...
  if (N >= 4096 && !ranges_overlap(c, bytes, a, bytes) &&
...
int alkhawarizm_metal_cpu_matmul_nn(void *C, const void *A, const void *B,
  ...
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, M, N, K, alpha,
              (const float *)A, K, (const float *)B, N, beta, (float *)C, N);
  return 0;
}
...
int alkhawarizm_metal_cpu_matmul_tb(void *C, const void *A, const void *B,
  ...
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, M, N, K, alpha,
              (const float *)A, K, (const float *)B, K, beta, (float *)C, N);
  return 0;
}
...
int alkhawarizm_metal_cpu_matvec_rows(void *y, const void *A, const void *x,
                                      ...
                                      float beta) {
  cblas_sgemv(CblasRowMajor, CblasNoTrans, rows, cols, alpha, (const float *)A,
              lda, (const float *)x, 1, beta, (float *)y, 1);
  return 0;
}
...
int alkhawarizm_metal_cpu_matvec_cols(void *y, const void *A, const void *x,
                                      int rows, int cols, int lda, float alpha,
                                      float beta) {
  cblas_sgemv(CblasRowMajor, CblasTrans, rows, cols, alpha, (const float *)A,
              lda, (const float *)x, 1, beta, (float *)y, 1);
  return 0;
}
...
int alkhawarizm_metal_cpu_rmsnorm(void *out, const void *x, const void *weight,
  ...
  size_t bytes = (size_t)N * sizeof(float);
  if (!addOne && N >= 256 && !ranges_overlap(oi, bytes, xi, bytes) &&
...
int alkhawarizm_metal_cpu_silu_ffn(void *out, const void *gate, const void *up,
  ...
  if (N >= ALKHAWARIZM_CPU_FFN_VECTOR_MIN_ELEMENTS &&
