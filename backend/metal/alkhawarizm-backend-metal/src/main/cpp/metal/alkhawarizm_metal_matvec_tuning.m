/**
 * alkhawarizm_metal_matvec_tuning.m — matvec dispatch policy and autotune
 * state.
 */
...
static NSMutableDictionary<NSString *, NSNumber *> *g_matvec_thread_choices =
    nil;

BOOL alkhawarizm_metal_prefer_matvec_128(int K, int max_output) {
  const char *override = getenv("ALKHAWARIZM_METAL_MATVEC_THREADS");
  ...
  int max_inner = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_MATVEC_128_MAX_INNER", 768);
  int max_out = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_MATVEC_128_MAX_OUTPUT", 4096);
...
BOOL alkhawarizm_metal_use_matvec_128(id<MTLComputePipelineState> pipeline128,
  ...
  return pipeline128 != nil &&
         alkhawarizm_metal_prefer_matvec_128(K, max_output);
}

BOOL alkhawarizm_metal_should_use_bf16_matvec_x4(int K, int max_output) {
  if (alkhawarizm_metal_env_truthy(
          "ALKHAWARIZM_METAL_DISABLE_BF16_MATVEC_X4")) {
    return NO;
  }
  const char *override = getenv("ALKHAWARIZM_METAL_MATVEC_THREADS");
  if (override != NULL && override[0] != '\0') {
  ...
  int min_inner = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_BF16_MATVEC_X4_MIN_INNER", 512);
  int min_out = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_BF16_MATVEC_X4_MIN_OUTPUT", 1024);
  ...
  int max_out = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_BF16_MATVEC_X4_MAX_OUTPUT", 8192);
...
BOOL alkhawarizm_metal_should_use_bf16_matvec_x8(int K, int max_output) {
  ...
  int min_inner = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_BF16_MATVEC_X8_MIN_INNER", 512);
  int min_out = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_BF16_MATVEC_X8_MIN_OUTPUT", 16384);
  int max_out = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_BF16_MATVEC_X8_MAX_OUTPUT", 0);
  return (min_inner <= 0 || K >= min_inner) &&
         (min_out <= 0 || max_output >= min_out) &&
         (max_out <= 0 || max_output <= max_out);
}
...
BOOL alkhawarizm_metal_should_use_bf16_pair_simd_reduction(int K,
                                                           int max_output) {
  if (alkhawarizm_metal_env_truthy(
          "ALKHAWARIZM_METAL_DISABLE_BF16_PAIR_SIMD")) {
    return NO;
  }
  if (alkhawarizm_metal_env_truthy("ALKHAWARIZM_METAL_ENABLE_BF16_PAIR_SIMD")) {
    return YES;
  }
  const char *override = getenv("ALKHAWARIZM_METAL_MATVEC_THREADS");
  if (override != NULL && override[0] != '\0') {
  ...
  int min_inner = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_BF16_PAIR_SIMD_MIN_INNER", 1024);
  int min_out = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_BF16_PAIR_SIMD_MIN_OUTPUT", 4096);
  int max_out = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_BF16_PAIR_SIMD_MAX_OUTPUT", 0);
  return (min_inner <= 0 || K >= min_inner) &&
         (min_out <= 0 || max_output >= min_out) &&
         (max_out <= 0 || max_output <= max_out);
}
...
BOOL alkhawarizm_metal_should_use_fused_gated_ffn_matvec(int is_bf16,
  ...
  if (alkhawarizm_metal_env_truthy(
          "ALKHAWARIZM_METAL_DISABLE_FUSED_GATED_FFN_MATVEC")) {
    return NO;
  }
  if (alkhawarizm_metal_env_truthy(
  ...
  if (alkhawarizm_metal_env_truthy(
          "ALKHAWARIZM_METAL_DISABLE_AUTO_FUSED_GATED_FFN_MATVEC")) {
    return NO;
  }
...
NSUInteger alkhawarizm_metal_cached_matvec_threads(NSString *key) {
  @synchronized([MPSMatrixVectorMultiplication class]) {
    NSNumber *cached = g_matvec_thread_choices != nil
                           ? [g_matvec_thread_choices objectForKey:key]
                           : nil;
    if (cached != nil) {
      NSUInteger value = (NSUInteger)[cached unsignedIntegerValue];
      if (value == ALKHAWARIZM_MATVEC_THREADS_128 ||
          value == ALKHAWARIZM_MATVEC_THREADS_256) {
        return value;
      }
    }
  }
...
void alkhawarizm_metal_cache_matvec_threads(NSString *key, NSUInteger threads) {
  if (threads != ALKHAWARIZM_MATVEC_THREADS_128 &&
      threads != ALKHAWARIZM_MATVEC_THREADS_256) {
    return;
  }
  @synchronized([MPSMatrixVectorMultiplication class]) {
...
NSUInteger alkhawarizm_metal_forced_matvec_threads(void) {
  ...
  long parsed = strtol(override, NULL, 10);
  if (parsed == 128)
    return ALKHAWARIZM_MATVEC_THREADS_128;
  if (parsed == 256)
    return ALKHAWARIZM_MATVEC_THREADS_256;
  return 0;
}

BOOL alkhawarizm_metal_matvec_autotune_enabled(int K, int max_output,
                                               BOOL can128) {
  if (!can128 || alkhawarizm_metal_forced_matvec_threads() != 0) {
    return NO;
  }
  if (alkhawarizm_metal_env_truthy(
          "ALKHAWARIZM_METAL_DISABLE_MATVEC_AUTOTUNE")) {
    return NO;
  }
  ...
  int max_inner = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_MATVEC_AUTOTUNE_MAX_INNER", 2048);
  int max_out = alkhawarizm_metal_env_int_or_default(
      "ALKHAWARIZM_METAL_MATVEC_AUTOTUNE_MAX_OUTPUT", 4096);
  return (max_inner <= 0 || K <= max_inner) &&
         (max_out <= 0 || max_output <= max_out);
}
...
NSUInteger alkhawarizm_metal_default_matvec_threads(
  ...
  NSUInteger forced = alkhawarizm_metal_forced_matvec_threads();
  if (forced == ALKHAWARIZM_MATVEC_THREADS_128 && pipeline128 != nil) {
    return ALKHAWARIZM_MATVEC_THREADS_128;
  }
  if (forced == ALKHAWARIZM_MATVEC_THREADS_256) {
    return ALKHAWARIZM_MATVEC_THREADS_256;
  }
  return alkhawarizm_metal_use_matvec_128(pipeline128, K, max_output)
...
BOOL alkhawarizm_metal_matvec_autotune_prefers_128(uint64_t nanos128,
                                                   uint64_t nanos256) {
  float margin = alkhawarizm_metal_env_float_or_default(
      "ALKHAWARIZM_METAL_MATVEC_AUTOTUNE_MARGIN", 0.03f);
  if (margin < 0.0f)
    margin = 0.0f;
...
void alkhawarizm_metal_log_matvec_autotune(const char *op, int K, int N0,
                                           ...
                                           uint64_t nanos256,
                                           NSUInteger selected) {
  if (!alkhawarizm_metal_env_truthy("ALKHAWARIZM_METAL_MATVEC_DEBUG") &&
      !alkhawarizm_metal_env_truthy(
          "ALKHAWARIZM_METAL_MATVEC_AUTOTUNE_DEBUG")) {
    return;
  }
  fprintf(stderr,
          "[alkhawarizm-metal] matvec autotune op=%s K=%d N0=%d N1=%d N2=%d "
          "t128=%.3fms t256=%.3fms selected=%lu\n",
          op, K, N0, N1, N2, (double)nanos128 / 1000000.0,
          (double)nanos256 / 1000000.0, (unsigned long)selected);
}
