 ...
 * Tensor out = vit.forward(image); // [B,3,224,224] → [B,1000]
 * }</pre>
 */
public final class ViT {
    ...
    static final class ViTModel extends NNModule {
        ...
        private final Tensor clsToken;  // [1, 1, dModel]
        ...
        ViTModel(int imgSize, int patchSize, int inChannels, int dModel,
            ...
            this.clsToken = register("cls_token", Tensor.randn(1, 1, dModel));
        ...
        public Tensor forward(Tensor x) {
            ...
            float[] patches = extractPatches(x.toFloatArray(), B, C, H, W, P, G);
            Tensor tokens = patchEmbed.forward(
                Tensor.of(patches, B, numPatches, C * P * P));

            // Prepend CLS token: [B, numPatches+1, dModel]
            Tensor cls = Tensor.of(clsToken.toFloatArray().clone(), B, 1, dModel);
            ...
            for (int b = 0; b < B; b++)
                System.arraycopy(tokens.toFloatArray(), b * (numPatches + 1) * dModel,
                                 clsOut, b * dModel, dModel);
            return head.forward(Tensor.of(clsOut, B, dModel));
        }
