...
import tech.kayys.alkhawarizm.core.tensor.TensorOps;
import tech.kayys.alkhawarizm.core.nn.RMSNorm;
 ...
 * Tensor logits = ((T5.T5Model) t5).forward(encInput, decInput);
 * }</pre>
 */
public final class T5 {
    ...
    public static final class T5Model extends NNModule {
        ...
        private final RMSNorm encNorm, decNorm;
        ...
        T5Model(int dModel, int nHeads, int nLayers, int dFF, int vocabSize) {
            ...
            this.encNorm = register("enc_norm", new RMSNorm(dModel, 1e-6f));
            this.decNorm = register("dec_norm", new RMSNorm(dModel, 1e-6f));
         ...
         */
        public Tensor forward(Tensor encInput, Tensor decInput) {
            // Encode
            Tensor enc = encInput;
        ...
        @Override
        public Tensor forward(Tensor x) { return forward(x, x); }
    }
    ...
    static final class T5Block extends NNModule {
        ...
        private final RMSNorm norm1, norm2, norm3;
        ...
        T5Block(int dModel, int nHeads, int dFF, boolean isDecoder) {
            ...
            this.norm1 = register("norm1", new RMSNorm(dModel, 1e-6f));
            this.norm2 = register("norm2", new RMSNorm(dModel, 1e-6f));
            ...
            if (isDecoder) {
                this.norm3 = register("norm3", new RMSNorm(dModel, 1e-6f));
        ...
        Tensor forward(Tensor x, Tensor encOut) {
            // Self-attention
        ...
        @Override public Tensor forward(Tensor x) { return forward(x, null); }

        private Tensor attention(Tensor q, Tensor kv, boolean cross) {
            long[] s = q.shape().dims();
            int B = (int)s[0], T = (int)s[1], D = (int)s[2];
            float scale = (float)(1.0 / Math.sqrt(headDim));

            Tensor K = cross ? xwK.forward(kv) : wK.forward(q);
            Tensor V = cross ? xwV.forward(kv) : wV.forward(q);
            Tensor Q = cross ? xwQ.forward(q)  : wQ.forward(q);

            Tensor scores = TensorOps.einsum("bhid,bhjd->bhij",
                reshape4d(Q, B, T, nHeads, headDim),
                reshape4d(K, B, (int)K.shape().dim(1), nHeads, headDim)).mul(scale);
            Tensor attn = scores.softmax();
            Tensor out  = TensorOps.einsum("bhij,bhjd->bhid", attn,
                reshape4d(V, B, (int)V.shape().dim(1), nHeads, headDim));
