 ...
 *
 * <p>
 * Stores model weights (SafeTensors), metadata, and version history.
 * Supports tagging, aliasing (e.g. "production", "staging"), and rollback.
 *
 * <h3>Example</h3>
 * 
 * <pre>{@code
 * var registry = new ModelRegistry(Path.of("~/.alkhawarizm/registry"));
 *
 ...
 * Map<String, Tensor> weights = registry.load("sentiment-classifier", "production");
 * }</pre>
 */
public final class ModelRegistry {
    ...
    public String register(String name, NNModule model, Map<String, String> metadata)
        ...
        var stateDict = model.namedParameters();
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            sb.append(String.format("\"%s\":{\"shape\":\"%s\",\"numel\":%d}",
                    e.getKey(),
                    Arrays.toString(e.getValue().shape().dims()),
                    e.getValue().numel()));
            if (i < entries.size() - 1)
                sb.append(",");
        }
     ...
     */
    public Map<String, Tensor> load(String name, String tagOrVersion) throws IOException {
